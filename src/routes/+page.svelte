<script lang="ts">
  ...
  const get_expected_stat_increases = (
    ...
    let remaining_levels = final_level;
    if (current_class === new_class && final_level >= current_level) {
      remaining_levels = final_level - current_level;
    }
  ...
  const add_entry = (e: SubmitEvent) => {
    ...
    if (level_ > 20 && class_ !== 'Thief' && class_ !== 'Dancer') {
    ...
    if (class_ === current_class && level_ < current_level) {
      if (current_class_data.is_base_class) {
        errors = [...errors, 'Unit cannot level backwards.'];
      } else if (
        ((class_ === 'Thief' || class_ === 'Dancer') && current_level < 40) ||
        current_level < 20
      ) {
        errors = [
          ...errors,
          'Make sure your unit is at the max level before resealing into the same class.'
        ];
      }
    }
  ...
  const delete_entry = (name_: string, i: number) => {
    ...
    if (
      ...
      if (next_entry.final_level < curr_entry.final_level) {
        errors = [
          ...errors,
          'Deleting this entry is invalid because units must hit their max level to reseal into the same advanced class.'
        ];
      }
    }
