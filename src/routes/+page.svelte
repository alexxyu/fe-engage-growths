<script lang="ts">
	...
	const total_growths = (ug: number[], cg: number[]) => ug.map((g, i) => g + cg[i]);

	let unit_ = "Alear";
	let class_ = "Dragon Child";
</script>
...
<section>
	...
	<select id="class-select" bind:value={class_}>
		{#each Object.keys(class_growths) as name}
			<option>{name}</option>
		{/each}
	</select>
	<table>
		...
		<tbody>
			...
			<tr>
				<th>Total</th>
				{#each total_growths(unit_growths[unit_], class_growths[class_]) as tg}
					<td class="{ tg >= 40 ? 'excellent-growth' : '' }">{tg}</td>
				{/each}
			</tr>
		</tbody>
	</table>
